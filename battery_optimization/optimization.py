...
class BatteryOptimization:
    ...
    def define_constraints(self, initial_soc: float):
        ...
        self.constraints += [
            ...
            - self.discharge[0] / self.battery.discharge_efficiency
        ]
        ...
        for t in range(self.ts):
            ...
            # power balance
            self.constraints += [
                self.grid_import[t] + self.system.solar[t] + self.discharge[t]
                == self.system.load[t] + self.charge[t]
            ]
    ...
    def plot_results(self, results):
        ...
        fig.add_trace(
            go.Scatter(
                x=time,
                y=results["grid_import_schedule"],
                mode="lines+markers",
                name="Grid Import (kW)",
            )
        )
        ...
        fig.update_layout(
            title="Battery Optimization Results",
